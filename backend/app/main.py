...
async def chat_simulate(req: ChatRequest):
    ...
    if commodity in ("crude", "both") and c_deficit > 0:
        ...
        if constraints.get("is_bottlenecked"):
            result["deliberation_log"].append(
                f"[Agent 2 - Quant] Infrastructure assessment: {port} bottlenecked! "
            ...
            result["deliberation_log"].append(
                f"[Agent 2 - Quant] {port} infrastructure: CLEAR. Capacity sufficient."
            )
        ...
        result["deliberation_log"].append(
            f"[Agent 3 - Trader] Scanned 22-grade global assay DB. "
        ...
        if unmet > 0.01:
            result["deliberation_log"].append(
                f"[Agent 4 - Governor] ⚠ CAPACITY GAP: Physical reserve capacity exhausted. "
                f"Max drawdown: {round(drawdown_total, 2)} MMT. Unmet deficit: {round(unmet, 2)} MMT — "
                f"requires emergency spot procurement."
            )
        else:
            result["deliberation_log"].append(
                f"[Agent 4 - Governor] Phase I MILP solver completed. "
    ...
    if commodity in ("gas", "both") and g_deficit > 0:
        ...
        if supplier_match.get("top_suppliers"):
            ...
            result["deliberation_log"].append(
                f"[Agent 3 - Trader] LNG market scan complete. "
        ...
        result["deliberation_log"].append(
            f"[Agent 4 - Governor] Gas distribution plan: "
    ...
    result["deliberation_log"].append(
        "[Agent 6 - Critic] Red-teaming the proposed plan for hidden vulnerabilities..."
    )
    ...
    if n_warn:
        ...
        result["deliberation_log"].append(
            f"[Agent 6 - Critic] ALERT: {n_warn} vulnerabilities found "
            f"({sev_list.count('CRITICAL')} CRITICAL, {sev_list.count('HIGH')} HIGH, "
            f"{sev_list.count('MEDIUM')} MEDIUM). Review before authorizing."
        )
    else:
        result["deliberation_log"].append(
            "[Agent 6 - Critic] Plan verified. No critical vulnerabilities detected. Safe to authorize."
        )
    ...
    result["deliberation_log"].append(
        f"[Agent 5 - War Room] Composite Supply Chain Risk Index: "
